...
set_silent(Flag) :- must_be(boolean, Flag),
                    retractall(silent(_)),
                    assertz(silent(Flag)).

current_silent(Flag) :- ( silent(Value) -> Flag = Value ; Flag = false ).

with_silent_mode(Flag, Goal) :- current_silent(Prev),
                                setup_call_cleanup(set_silent(Flag),
                                                   Goal,
                                                   set_silent(Prev)).
