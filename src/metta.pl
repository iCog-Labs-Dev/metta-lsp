...
%%% Browser runtime feature guards: %%%
unsupported_feature(Feature, Context, Message) :- throw(error(unsupported_feature(Feature), context(Context, Message))).

%%% Python bindings (disabled in browser runtime v1): %%%
'py-call'(_, _) :- unsupported_feature(py_call, 'py-call', 'Python interop is not supported in browser runtime v1.').
'py-call'(_, _, _) :- unsupported_feature(py_call, 'py-call', 'Python interop is not supported in browser runtime v1.').
...
'import!'(_, _, _) :- throw(error(browser_policy(imports_disabled),
                                  context('import!', 'import! is disabled in browser runtime v1.'))).
